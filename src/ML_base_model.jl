...
function __reward(mdp::OriginalMDP,s::MLState,a::MLAction)
	...
	cost = 0.
	if a.vel > 0
		cost += mdp.accel_cost*a.vel
	elseif a.vel < 0
		cost -= mdp.decel_cost*a.vel
	end
	if abs(a.lane_change) != 0
		cost += mdp.lanechange_cost
		if (a.lane_change == -1) && (s.agent_pos == 1)
			cost += mdp.invalid_cost
		elseif (a.lane_change == 1) && (s.agent_pos == mdp.nb_col)
			cost += mdp.invalid_cost
		end
	end
	if mod(s.agent_pos,2) == 0 #on a lane divider--an
		cost += mdp.lineride_cost
	end
...
function next(rng::AbstractRNG,mdp::OriginalMDP,s::MLState,a::MLAction)
  pp = mdp.phys_param
  dt = pp.dt
  nbcols = 2*pp.nb_lanes-1
  BEHAVIORS = mdp.BEHAVIORS
